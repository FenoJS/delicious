...
exports.resize = async function (req, res, next) {
  ...
  const exstension = req.file.mimetype.split('/')[1];
  req.body.photo = `${uuid.v4()}.${exstension}`;
...
exports.getStoresByTag = async (req, res) => {
  const { tag } = req.params; // es6 destructuring
...
exports.heartStore = async (req, res) => {
  ...
  const operator = hearts.includes(req.params.id) ? '$pull' : '$addToSet'; // Toggle mondgodb request
